...
export default class ASurveyRunService {
  ...
  static async run({ surveyId, action, body, session }) {
    ...
    static async savePageAnswers({ steps, wrappers, body, responseId, session, isInRotation }) {
      ...
      const keysToDelete = this.computeKeysToDelete(step, rawValue, mainValue,oldOptionCode);
  ...
  static computeKeysToDelete(step, rawValue, mainValue, oldOptionCode = undefined) {
    const keysToDelete = [];
  ...
  static pushCurrentStepToHistory(session, step, isRotation,wrapper=null) {
    ...
    //       let mainValue = this.getMainValue(step, body, rawValue);
    //       console.log("mainnnnn value",mainValue)
    //       // if (!normalized || Object.keys(normalized).length === 0 || mainValue === null || mainValue === '' || (Array.isArray(mainValue) && mainValue.length === 0)) {
    //       //   return;
    //       // }
    //       // if(step.type=== "accordion" ){
    //       //   this.saveSessionAnswers({ step, normalized, mainValue, session, wrapper, isInRotation });
    //       // }
    //       // --- 1. Calculer les clés à supprimer ---
    //       // const keysToDelete = this.computeKeysToDelete(step, session.answers, body);
    //       const previousSelected = session.answers[step.id]; // ex: '7'
    // const oldOptionCode = Array.isArray(previousSelected) ? previousSelected[0] : previousSelected;
    ...
    //       // --- 2. Nettoyer la session et ajouter d’autres clés à supprimer si nécessaire ---
    //      // const previousSelected = Array.isArray(session.answers[step.id]) ? [...session.answers[step.id]] : [];
    //       await this.cleanupSession(step, session, mainValue, previousSelected);
          
    //       // Après cleanup, supprimer toute clé devenue vide dans session
    ...
    //   //       this.saveSessionAnswers({ step, normalized, mainValue, session, wrapper, isInRotation });
    //   //       this.saveStepPrecisions({ step, rawValue, mainValue, sessionAnswers: session.answers });
    //   //     //  SessionCleanupUtils.cleanObject(session.answers);
    ...
    //     //         const dbKey = question.id_db;
    //   //         // Débogage : afficher les IDs et la valeur actuelle
    //   //         console.log("Processing question:", question.id, "DB key:", dbKey);
      
    //   //         const oldValue = sessionAnswers[step.id]?.[question.id] ?? sessionAnswers[question.id];
      
    //   //         console.log("Old value:", oldValue);
      
    //   //         if (isEmpty(oldValue)) {
    //   //             console.log("-> Empty! Adding to keysToDelete:", dbKey);
    //   //             keysToDelete.push(dbKey);
    //   //         }
    //   //     };
      
    //   //     // Cas 1 : Step avec sections
    //   //     if (step.sections && Array.isArray(step.sections)) {
    //   //         console.log("Step has sections:", step.sections.length);
    //   //         step.sections.forEach((section, sIndex) => {
    //     //             if (section.questions && Array.isArray(section.questions)) {
    //   //                 console.log(`Section ${sIndex} has questions:`, section.questions.length);
    //   //                 section.questions.forEach(processQuestion);
    //   //             } else {
    //   //                 console.log(`Section ${sIndex} has no questions`);
    //   //             }
    //   //         });
    //   //     }
    //   //     // Cas 2 : Step seul (directement une question)
    //   //     else if (step.id_db) {
    //   //         console.log("Step is a single question:", step.id);
    //   //         processQuestion(step);
    //   //     } else {
    //   //         console.log("Step has no sections and no id_db!");
    //   //     }
